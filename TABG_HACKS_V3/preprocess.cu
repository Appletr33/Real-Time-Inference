...
__global__ void warpaffine_kernel_bgra(
    uint8_t* src,           // Source image on device (BGRA format)
    ...
    if (src_x <= -1 || src_x >= src_width || src_y <= -1 || src_y >= src_height) {
        ...
        uint8_t const_value[] = { const_value_st, const_value_st, const_value_st, const_value_st };
        ...
        if (y_low >= 0) {
            if (x_low >= 0)
                v1 = src + y_low * src_line_size + x_low * 4;
            // Top-right pixel
            if (x_high < src_width)
                v2 = src + y_low * src_line_size + x_high * 4;
        }

        // Bottom-left and Bottom-right pixels
        if (y_high < src_height) {
            if (x_low >= 0)
                v3 = src + y_high * src_line_size + x_low * 4;
            if (x_high < src_width)
                v4 = src + y_high * src_line_size + x_high * 4;
        }
        ...
        // Alpha channel is vX[3], but typically not used for inference
    }

    // Convert from BGR(A) to RGB by swapping channels
    float t = c2; // Swap R and B channels
    c2 = c0;
    c0 = t;
...
// Host function to perform CUDA-based preprocessing from device memory
void cuda_preprocess_from_device(
    uint8_t* src_dev_ptr, // Source image data in device memory (BGRA)
    int src_width,        // Source image width
    int src_height,       // Source image height
    ...
    // No need to copy data; it's already in device memory
    ...
    // Launch the warp affine kernel for BGRA input
    warpaffine_kernel_bgra << <blocks, threads, 0, stream >> > (
        src_dev_ptr,         // Source image on device (BGRA format)
        src_width * 4,       // Source line size (bytes per row)
